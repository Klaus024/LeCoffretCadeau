...
document.getElementById('adminSignupForm').addEventListener('submit', async (e) => {
  ...
  const full_name = document.getElementById('adminSignupName').value;
  ...
  // Créer l'entrée admin (rôle = admin par défaut)
  ...
  if (insertError) {
    showNotification('Erreur profil admin : ' + insertError.message, 'error');
    return;
  }

  showNotification('Compte admin créé ! Vérifiez vos emails.', 'success');
}); 